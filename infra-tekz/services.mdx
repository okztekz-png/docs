...
A publicação dos serviços que rodam na VM `services` geralmente segue este fluxo:
...
<Note>
  Novos serviços públicos normalmente devem ser publicados via Traefik, evitando NAT direto para portas específicas sempre que possível.
</Note>
...
Abaixo estão as principais stacks documentadas no Portainer.

| Stack | Ambiente | Status / Observação |
| --- | --- | --- |
| `chatwoot` | Swarm | Atendimento / produção |
| `chatwoot_kanban` | Swarm | Addon para Chatwoot |
| `dify-plugin-daemon` | Swarm | Componente do Dify |
| `dify_api` | Swarm | API do Dify |
| `dify_sandbox` | Swarm | Sandbox do Dify |
| `dify_web` | Swarm | Interface web do Dify |
| `dify_worker` | Swarm | Worker do Dify |
| `docmost_tekz` | Swarm | Documentação antiga, prevista para remoção |
| `evo-go-connector` | Swarm | Conector Evolution Go / Chatwoot |
| `evogoInactive` | Swarm | Inativo / legado |
| `evolution_v2Inactive` | Swarm | Inativo / legado |
| `evolutiongo` | Swarm | Evolution Go em teste/uso |
| `n8n_editor` | Swarm | Interface do n8n |
| `n8n_mcp_api` | Swarm | API MCP do n8n |
| `n8n_webhook` | Swarm | Webhooks do n8n |
| `n8n_worker` | Swarm | Workers do n8n |
| `noc-tv` | Compose | Dashboard customizado para TV |
| `passbolt` | Swarm | Cofre de senhas privado |
| `pgadmin` | Swarm | Administração PostgreSQL |
| `pgvector` | Swarm | Banco vetorial / IA |
| `portainer` | Swarm | Gerenciamento Docker |
| `postgres` | Swarm | Banco PostgreSQL compartilhado |
| `redis` | Swarm | Cache/fila |
| `report-service` | Swarm | Serviço de relatórios |
| `serplan_proxy_temp` | Swarm | Proxy temporário |
| `tekz_proxy_temp` | Swarm | Proxy temporário |
| `traefik` | Swarm | Proxy reverso |
| `uptime_kumaInactive` | Swarm | Inativo; migrado para Oracle Cloud |
