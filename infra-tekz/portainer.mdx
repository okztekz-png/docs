...
O Portainer administra diversas stacks da infraestrutura interna da Tekz.

| Stack | Ambiente | Função / Observação |
| --- | --- | --- |
...
| `chatwoot_kanban` | Addon para Chatwoot |
| `evolutiongo` | Evolution Go em teste/uso |
| `evo-go-connector` | Integração Evolution Go \+ Chatwoot |
| `n8n_editor` | Interface de automação |
| `n8n_webhook` | Webhooks de automação |
...
| `noc-tv` | Dashboard operacional |

### Uso eventual / baixa frequência
...
| `pgvector` | Banco vetorial / IA |
| `pgadmin` | Administração PostgreSQL |

### Legado / inativo / temporário

| Stack | Observação |
| --- | --- |
| `docmost_tekz` | Será removido após migração para Mintlify |
...
<Note>
  As classificações acima devem ser revisadas conforme os serviços forem confirmados como ativos, inativos ou removidos.
</Note>
