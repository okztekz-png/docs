...
| Tipo | A |
| Aponta para | `179.51.153.51` |
| Destino interno no firewall | `172.16.0.253` |
| Serviço interno | Traefik |
| Portas | `80` e `443` |

## NAT no OPNsense

O firewall OPNsense encaminha as portas públicas `80` e `443` para a VM `services`.
...
<Note>
  A partir desse NAT único, o Traefik consegue publicar vários serviços diferentes usando domínios separados.
</Note>
...
| Serviço | Publicação | Observação |
| --- | --- | --- |
| Proxmox | NAT direto `8006` → `172.16.0.230:8006` | Acesso administrativo sensível |
| Elastic | NAT direto `9200` → `172.16.0.152:9200` | Serviço legado |
| Nextcloud | NAT direto `8086` → `172.16.0.118:443` | Publicação externa direta |
| UniFi | NAT direto `8443/8080` → `172.16.0.250` | Controller e adoção |
| n8n legado | NAT direto `5678` → `172.16.0.152:5678` | Serviço antigo |
| Chatwoot legado | NAT direto `8085` → `172.16.0.152:3000` | Serviço antigo |
| Evolution API legado | NAT direto `8081` → `172.16.0.152:8080` | Serviço antigo |
| Gerador agente HelpTekz | NPM Oracle → `179.51.153.51:8899` | Geração de `.exe` |
| Uptime Kuma | Oracle Cloud | Migrado para ambiente externo |
